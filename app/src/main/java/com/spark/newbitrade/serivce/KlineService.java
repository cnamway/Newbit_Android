...
public class KlineService extends Service {
    ...
    public static final String ip = "47.74.21.247";
//    public static final String ip = "192.168.2.35";
    private static final int port = 28903;
    ...
    private static final String lock = "klineLock";
    private int code;
    ...
    public void onGetMessage(SocketMessage message) {
        ...
        if (code == GlobalConstant.CODE_KLINE) {
            try {
                String json = "";
                ...
                LogUtils.i("发送的K线指令==" + message.getCmd() + ",,,内容===" + json);
    ...
    private void releaseSocket() {
        ...
        try {
            ...
            if (socket.isConnected()) {
                ...
                toRequest(ISocket.CMD.SUBSCRIBE_THUMB, null);
                LogUtils.i("K线指令重连成功");
            }
        } catch (IOException e) {
            e.printStackTrace();
            ...
            synchronized (lock) {
                if (socket == null || !socket.isConnected()) {
                    try {
                        ...
                        LogUtils.i("k线 socket   连接成功");
                    } catch (IOException e) {
                        LogUtils.i("k线 socket   连接异常");
                        e.printStackTrace();
                        isOpen = false;
                        socket = null;
                    }
    ...
    private void dealResponse(DataInputStream dis) throws Exception {
        ...
        LogUtils.v("返回k线指令==" + "cmd====" + cmd + ",,,返回数据==" + str);
        if (responseCode == 200) {
            EventBus.getDefault().postSticky(new SocketResponse(cmd, str, code));
        }
    }
