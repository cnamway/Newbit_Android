...
public class AdsAdapter extends BaseQuickAdapter<Ads, BaseViewHolder> {
    ...
    protected void convert(final BaseViewHolder helper, Ads item) {
        ...
        if (payMode.toLowerCase().contains(GlobalConstant.PAYPAL))
            helper.setVisible(R.id.ivPalpay, true);
        ...
        if (item.getTradeType() == 0)
            helper.setText(R.id.tvLimit, MyApplication.getApp().getString(R.string.limit) + item.getMinLimit() + "~" + item.getMaxLimit() + item.getLocalCurrency());
        else
            helper.setText(R.id.tvLimit, "无限额");
    }
}
