package com.spark.newbitrade.ui.kchart;
...
import com.spark.newbitrade.entity.DepthDataBean;
    ...
    private void init(AttributeSet attrs) {
        ...
        if (typedArray != null) {
            try {
                ...
                mDotRadius = typedArray.getDimensionPixelSize(R.styleable.DepthMapView_dot_radius, DpPxUtils.dip2px(getContext(), mDotRadius));
                mCircleRadius = typedArray.getDimensionPixelSize(R.styleable.DepthMapView_circle_radius, DpPxUtils.dip2px(getContext(), mCircleRadius));
                mBackgroundColor = typedArray.getColor(R.styleable.DepthMapView_background_color, CommonUtils.getColor(getContext(), R.color.depth_background));
                mBuyLinePaint.setStrokeWidth(typedArray.getDimensionPixelSize(R.styleable.DepthMapView_line_width, DpPxUtils.dip2px(getContext(), 1.5f)));
                mSellLinePaint.setStrokeWidth(typedArray.getDimensionPixelSize(R.styleable.DepthMapView_line_width, DpPxUtils.dip2px(getContext(), 1.5f)));
                mTextPaint.setColor(typedArray.getColor(R.styleable.DepthMapView_text_color, CommonUtils.getColor(getContext(), R.color.depth_text_color)));
                mTextPaint.setTextSize(typedArray.getDimension(R.styleable.DepthMapView_text_size, CommonUtils.getDimension(getContext(), R.dimen.sp_10)));
                mBuyLinePaint.setColor(typedArray.getColor(R.styleable.DepthMapView_buy_line_color, CommonUtils.getColor(getContext(), R.color.depth_buy_line)));
                mSellLinePaint.setColor(typedArray.getColor(R.styleable.DepthMapView_sell_line_color, CommonUtils.getColor(getContext(), R.color.depth_sell_line)));
                mBuyPathPaint.setColor(typedArray.getColor(R.styleable.DepthMapView_buy_path_color, CommonUtils.getColor(getContext(), R.color.depth_buy_path)));
                mSellPathPaint.setColor(typedArray.getColor(R.styleable.DepthMapView_sell_path_color, CommonUtils.getColor(getContext(), R.color.depth_sell_path)));
                mSelectorBackgroundPaint.setColor(typedArray.getColor(R.styleable.DepthMapView_selector_background_color, CommonUtils.getColor(getContext(), R.color.depth_selector)));
            } finally {
                typedArray.recycle();
            }
        }
...
//                    mIsLongPress = true;
    ...
    private void drawSelectorView(Canvas canvas, int position) {
        ...
        if (position < mDrawWidth) {
            ...
            mRadioPaint.setColor(CommonUtils.getColor(getContext(), R.color.depth_buy_line));
        } else {
            canvas.drawCircle(position, y, mCircleRadius, mSellLinePaint);
            mRadioPaint.setColor(CommonUtils.getColor(getContext(), R.color.depth_sell_line));
        }
        canvas.drawCircle(position, y, mDotRadius, mRadioPaint);

        String volume = getContext().getString(R.string.item_number) + ": " + getVolumeValue(mMapX.get(position).getVolume());
        String price = getContext().getString(R.string.text_entrust_price) + ": " + getValue(mMapX.get(position).getPrice());
        float width = Math.max(mTextPaint.measureText(volume), mTextPaint.measureText(price));
        Paint.FontMetrics metrics = mTextPaint.getFontMetrics();
        float textHeight = metrics.descent - metrics.ascent;

        int padding = DpPxUtils.dip2px(getContext(), 5);
        ...
        canvas.drawText(getContext().getString(R.string.item_number) + ": ",
                mDrawWidth - width / 2 + padding + mTextPaint.measureText(getContext().getString(R.string.item_number)), textHeight + 2, mTextPaint);
        canvas.drawText(getVolumeValue(mMapX.get(position).getVolume()), mDrawWidth + width / 2 + padding, textHeight + 2, mTextPaint);
        canvas.drawText(getContext().getString(R.string.text_entrust_price) + ": ",
                mDrawWidth - width / 2 + padding + mTextPaint.measureText(getContext().getString(R.string.text_entrust_price)), textHeight * 2 + padding, mTextPaint);
